...
extern "C" {
    ...
    /// The number of ticks until the [`StructureLab`] can use [`StructureLab::run_reaction`] or [`StructureLab::unboost_creep`] again.
    ///
    /// [Screeps documentation](https://docs.screeps.com/api/#StructureLab.cooldown)
    #[wasm_bindgen(method, getter)]
    pub fn cooldown(this: &StructureLab) -> u32;

    /// The [`Store`] of the lab, which can contain energy and one type of resource at a time.
    ///
    ...
    /// Reverse a reaction, splitting the compound in this [`StructureLab`] into its components in two other labs.
    ///
    ...
    /// Run a reaction, combining components from two other [`StructureLab`]s into a new compound in this lab.
    ///
    ...
    /// Unboost a [`Creep`], removing all boosts from its body and dropping [`LAB_UNBOOST_MINERAL`] per body part on the ground, with a cooldown equal to the total time to produce the removed boosts.
    ///
