...
extern "C" {
    ...
    /// The dead [`Creep`] or [`PowerCreep`] that this tombstone represents. Note that this object is not fully safe to use, and needs to be cast into the correct type.
    ///
    ...
    /// Object ID of the tombstone, which can be used to efficiently fetch a fresh reference to the object on subsequent ticks.
    ///
    /// [Screeps documentation](https://docs.screeps.com/api/#Tombstone.id)
    #[wasm_bindgen(method, getter)]
    pub fn id(this: &Tombstone) -> JsString;

    /// The [`Store`] of the tombstone, which contains any resources in the tombston.
    ///
