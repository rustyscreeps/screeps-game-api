...
//     objects::{
//         Attackable, ConstructionSite, Creep, Harvestable,
// SharedCreepProperties,         StructureController, StructureProperties,
...
//         // Has to be deconstructed manually to avoid converting strings from
// js to rust         let len: u32 = js_unwrap!(@{self.as_ref()}.body.length);
//         let mut body_parts: Vec<Bodypart> = Vec::with_capacity(len as usize);

//         for i in 0..len {
//             let boost_v = js!(const
// b=@{self.as_ref()}.body[@{i}].boost||null;return
// b&&__resource_type_str_to_num(b););             let boost = match boost_v {
//                 Value::Number(_) => {
//                     Some(ResourceType::try_from(boost_v).expect("Creep boost
// resource unknown."))                 }
//                 _ => None,
//             };
//             let part: Part =
// js_unwrap!(__part_str_to_num(@{self.as_ref()}.body[@{i}].type));             
