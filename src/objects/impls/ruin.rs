...
impl Ruin {
    ...
    pub fn structure_info(&self) -> (StructureType, RawObjectId, Option<String>) {
        (
            ...
            RawObjectId::from_packed_js_val(js_unwrap!(object_id_to_packed(@{self.as_ref()}.structure.id)))
                .expect("expected ruin structure's JavaScript id to be a 12-byte number encoded in hex"),
            (js! {
