...
pub mod season_5 {
    ...
    ///
    /// [`Thorium`]: crate::constants::ResourceType::Thorium
    pub fn thorium_decay_factor(thorium_amount: u32) -> u32 {
    ...
    #[cfg(test)]
    mod test {
        use super::{reactor_points_per_tick, thorium_decay_factor};

        #[test]
        fn decay_growth_formula() {
            assert_eq!(thorium_decay_factor(10), 1);
            assert_eq!(thorium_decay_factor(100), 2);
            assert_eq!(thorium_decay_factor(1000), 3);
        }

        #[test]
        fn score_growth_formula() {
